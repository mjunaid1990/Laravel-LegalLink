                ...
                <table id="agreement-table" class="table table-white-rows">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>#{{$model->id}}</td>
                            <td>{{$model->name}}</td>
                            <td>{{$model->tag_color}}</td>
                            <td>{{$model->description?Str::limit($model->description, 100):''}}</td>
                            <td>
                                @if($model->image)
                                <div class="td-avatar">
                                    <img src="{{asset('uploads/categories/'.$model->id.'/'.$model->image)}}" alt="" />
                                </div>
                                @endif
                            </td>
                            <td class="action">
                                ...
                                <div class="dropdown-menu dropdown-caret dropdown-menu-end py-0" aria-labelledby="navbarDropdownUser">
                                    <div class="bg-white dark__bg-1000 rounded-2">
                                        ...
                                        <a class="dropdown-item sidebar-modal" href="javascript:void(0);" data-url="/admin/categories/edit/{{$model->id}}">
                                            Edit
                                        </a>
                                        ...
                                        <a class="dropdown-item text-danger" data-method="DELETE" href="javascript:void(0)" 
                                           onclick="if(!confirm('Are you sure you want to delete?')){return false;} event.preventDefault();
                                            ...
                                            <form method="post" id="delete-form-{{$model->id}}" action="{{ route('admin.category.delete', $model->id) }}" enctype="multipart/form-data">
                                                @csrf
                                                @method('delete')
